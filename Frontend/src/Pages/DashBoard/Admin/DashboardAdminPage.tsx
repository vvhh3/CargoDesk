...
const stats = [
  {
    label: "Total Revenue", value: "$892K", change: "+18.2%", trend: "up",
    icon: DollarSign, color: "from-[#22C55E] to-[#16A34A]",
    chartData: [45, 52, 48, 61, 55, 67, 72, 68, 78, 85, 82, 92],
  },
  {
    label: "Total Orders", value: "5,690", change: "+12.5%", trend: "up",
    icon: ShoppingCart, color: "from-[#7C3AED] to-[#8B5CF6]",
    chartData: [320, 380, 350, 420, 390, 460, 510, 480, 550, 600, 580, 650],
  },
  {
    label: "Active Users", value: "2,847", change: "+8.7%", trend: "up",
    icon: Users, color: "from-[#3B82F6] to-[#2563EB]",
    chartData: [180, 195, 188, 210, 205, 225, 240, 235, 255, 270, 265, 285],
  },
  {
    label: "Avg. Order Value", value: "$157", change: "-2.3%", trend: "down",
    icon: TrendingUp, color: "from-[#F59E0B] to-[#D97706]",
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="flex-1 overflow-y-auto p-8">
                ...
                <div className="relative p-6 rounded-2xl bg-[#111113] border border-white/10 backdrop-blur-xl">
                  ...
                  <div className="h-12 flex items-end gap-1">
                    {stat.chartData.map((value, idx) => (
                      <div key={idx} className={`flex-1 rounded-t bg-linear-to-t ${stat.color} opacity-60`}
            ...
            <div className="col-span-2 p-6 rounded-2xl bg-[#111113] border border-white/10 backdrop-blur-xl">
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center gap-2">
                  <button className="px-3 py-1.5 rounded-lg bg-linear-to-r from-[#7C3AED] to-[#8B5CF6] text-sm">Revenue</button>
                  <button className="px-3 py-1.5 rounded-lg bg-white/5 hover:bg-white/10 text-sm text-zinc-400">Profit</button>
                  <button className="px-3 py-1.5 rounded-lg bg-white/5 hover:bg-white/10 text-sm text-zinc-400">Orders</button>
                </div>
              </div>
          ...
          <div className="grid grid-cols-3 gap-6 mb-8">
            <div className="col-span-2 p-6 rounded-2xl bg-[#111113] border border-white/10 backdrop-blur-xl">
              <div className="flex items-center justify-between mb-6">
                <div><h3 className="text-lg font-semibold mb-1">Top Performing Managers</h3><p className="text-sm text-zinc-400">Based on orders and revenue</p></div>
                <button className="text-sm text-[#7C3AED] hover:text-[#8B5CF6] transition-colors">View All</button>
              </div>
              <div className="space-y-4">
                {managerPerformance.map((manager, i) => (
                  <div key={i} className="flex items-center gap-4 p-4 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition-all group">
            ...
            <div className="p-6 rounded-2xl bg-[#111113] border border-white/10 backdrop-blur-xl">
                  ...
                  <div key={i} className="flex items-start gap-3 p-3 rounded-lg bg-white/5 border border-white/10">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        {activity.amount !== "-" && <span className="text-xs font-medium text-[#22C55E]">{activity.amount}</span>}
                      </div>
              ...
              <button className="w-full mt-4 py-2 rounded-lg bg-white/5 border border-white/10 hover:bg-white/10 transition-all text-sm text-zinc-400 hover:text-white">View All Activity</button>
            </div>
          </div>
