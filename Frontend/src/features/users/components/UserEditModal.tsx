...
export function UserEditModal({ user, onSelectedUser }: ModalUserProps) {
  ...
  const {
    ...
    defaultValues: { name: user?.name, lastName: user?.lastName, email: user?.email, companyName: user?.companyName },
    mode: "onBlur"
  })


  const editMutation = useMutation({ //это хук TanStack Query, предназначенный для изменения данных
    mutationFn: (data: Parameters<typeof usersApi.edit>[0]) => usersApi.edit(data),
    onSuccess: (res) => {
